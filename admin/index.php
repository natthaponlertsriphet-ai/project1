...
<div class="shadcn-card border border-amber-500/30 bg-zinc-900/90 shadow-xl shadow-amber-500/5 rounded-xl p-6 mt-8">
    ...
    <div class="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 xl:grid-cols-6 gap-3.5 p-5 bg-zinc-950 border border-zinc-900 rounded-lg">
        <?php foreach ($all_tables as $t): ?>
            <a href="index.php?action=toggle_status&id=<?php echo $t['id']; ?>" 
               onclick="toggleTableStatusRealtime(event, '<?php echo $t['id']; ?>', this)" 
               class="admin-map-btn block no-underline transition-transform duration-200 hover:-translate-y-0.5" 
               data-table-id="<?php echo $t['id']; ?>" 
               data-table-status="<?php echo $t['status']; ?>" 
               data-zone="<?php echo $t['zone']; ?>" 
               title="<?php echo t('Click to toggle status', 'คลิกเพื่อสลับสถานะโต๊ะ'); ?>">
...
function toggleTableStatusRealtime(event, tableId, el) {
    ...
    // 1. Instant Real-Time Optimistic UI Update (0ms latency, zero page refresh!)
    elements.forEach(linkEl => {
    ...
    // 2. Perform background AJAX update
